...
What this script does, in order:
  ...
  4. Run every free analysis/f*.py script over that output (unless
     --skip-analysis): F07/F08/F10/F11/F12/F14/F20/F02/F18, plus F13/F05
     if --enable-repeat-sampling was set (they need cached repeat-sampling
     hypotheses), plus F31's real-validation figures at the end.

Two further real experiments are opt-in ONLY, since each multiplies your
real Gemini API cost on top of the main run above:
  --run-corruption-suite        F19: generates real corrupted copies of
                                 your images (blur/noise/brightness/JPEG,
                                 3 severities each) and runs a second real
                                 pipeline pass against them, then compares.
  --run-repeatability            F21: runs the real pipeline
                                 --repeatability-runs times over the same
                                 images to measure real run-to-run label/
...
def ensure_dependencies():
    required = ["numpy", "scipy", "sklearn", "pandas", "PIL", "cv2", "requests", "tqdm", "matplotlib"]
    missing = []
    for mod in required:
        try:
            if mod == "sklearn":
    ...
    try:
        ...
        print(
            "[run.py] sentence-transformers not installed -- F05's embedding-dispersion baseline "
            "will be skipped automatically (needs internet to download a small model on first use). "
...
def run_analysis_scripts(sample_level_path, frozen_config_path, out_dir, enable_repeat_sampling):
    ...
    sample_level_resolved = str(Path(sample_level_path).resolve())

    scripts = [
        ["f07_bootstrap_ci.py", ["--input", sample_level_resolved, "--out-json", str((out_dir / "f07_bootstrap_ci.json").resolve())]],
        ["f08_spearman_matrix.py", ["--input", sample_level_resolved, "--out-csv", str((out_dir / "f08_spearman_matrix.csv").resolve())]],
        ["f10_safety_decomposition.py", ["--input", sample_level_resolved, "--frozen-config", str(Path(frozen_config_path).resolve()), "--out-json", str((out_dir / "f10_safety_decomposition.json").resolve())]],
        ["f11_paired_tests.py", ["--input", sample_level_resolved, "--out-json", str((out_dir / "f11_paired_tests.json").resolve())]],
        ["f12_classwise_diagnostics.py", ["--input", sample_level_resolved, "--out-csv", str((out_dir / "f12_classwise_diagnostics.csv").resolve())]],
        ["f14_image_quality_sweep.py", ["--input", sample_level_resolved, "--out-prefix", str((out_dir / "f14").resolve())]],
        ["f20_latency_cost.py", ["--input", sample_level_resolved, "--out-csv", str((out_dir / "f20_latency_cost.csv").resolve())]],
        ["f02_leakage_free_protocol.py", ["--input", sample_level_resolved, "--out-json", str((out_dir / "f02_leakage_free_protocol.json").resolve())]],
        ["f18_failure_analysis.py", ["--input", sample_level_resolved,
                                      "--out-confusion-csv", str((out_dir / "f18_confusion_pairs.csv").resolve()),
                                      "--out-coding-sheet", str((out_dir / "f18_failure_coding_sheet.csv").resolve())]],
    ]

    if enable_repeat_sampling:
        scripts.append(
            ["f13_threshold_budget_sensitivity.py", ["--input", sample_level_resolved, "--out-csv", str((out_dir / "f13_threshold_budget_sensitivity.csv").resolve())]]
        )
        scripts.append(
            ["f05_baseline_suite.py", ["--input", sample_level_resolved, "--out-csv", str((out_dir / "f05_baseline_comparison.csv").resolve())]]
        )
    else:
        print(
            "[run.py] Skipping F13 (threshold/budget sensitivity) and F05 (semantic-entropy/"
            "embedding-dispersion baselines): both need repeat-sampling data cached by "
            "--enable-repeat-sampling. Re-run with that flag, then run.py --skip-ingest "
            "--skip-run to just redo analysis."
        )
    ...
    # F31: real-validation figures, depends on F10's output existing above.
    safety_json = out_dir / "f10_safety_decomposition.json"
    fig_prefix = out_dir / "fig_real_validation"
    print(f"\n[run.py] Running f31_generate_figures.py ...")
    result = subprocess.run(
        [sys.executable, str(analysis_dir / "f31_generate_figures.py"),
         "--input", sample_level_resolved,
         "--safety-json", str(safety_json.resolve()),
         "--out-prefix", str(fig_prefix.resolve())],
        cwd=str(HERE),
    )
    if result.returncode != 0:
        print("[run.py] WARNING: f31_generate_figures.py exited with a nonzero code")


def run_corruption_suite(args, manifest_path, out_dir):
    """F19: opt-in only, since this multiplies real API cost by (corruption types x severities)."""
    corrupted_dir = out_dir / "corrupted_images"
    corrupted_manifest = out_dir / "manifest_corrupted.json"
    corrupted_sample_level = out_dir / "uavi_sample_level_corrupted.json"

    print(f"\n[run.py] F19: generating corrupted images into {corrupted_dir} ...")
    gen_args = [
        "--manifest", str(Path(manifest_path).resolve()),
        "--out-dir", str(corrupted_dir.resolve()),
        "--out-manifest", str(corrupted_manifest.resolve()),
    ]
    if args.corruption_types:
        gen_args += ["--corruptions"] + args.corruption_types
    if args.corruption_max_source_samples:
        gen_args += ["--max-samples", str(args.corruption_max_source_samples)]
    subprocess.run([sys.executable, str(HERE / "f19_generate_corruptions.py")] + gen_args, cwd=str(HERE), check=True)

    print(f"\n[run.py] F19: running the real pipeline against corrupted images "
          f"(this makes real, billed Gemini calls) ...")
    api_key = args.api_key or os.environ.get("GEMINI_API_KEY")
    pipeline_args = argparse.Namespace(
        manifest=str(corrupted_manifest), out=str(corrupted_sample_level), api_key=api_key,
        max_samples=None, tau_low=args.tau_low, tau_high=args.tau_high,
        enable_repeat_sampling=False, n_repeat=args.n_repeat, temperature=args.temperature,
    )
    run_pipeline.run(pipeline_args)

    print("\n[run.py] F19: comparing clean vs. corrupted results ...")
    subprocess.run(
        [sys.executable, str(HERE / "analysis" / "f19_robustness_analysis.py"),
         "--clean-input", str((out_dir / "uavi_sample_level.json").resolve()),
         "--corrupted-input", str(corrupted_sample_level.resolve()),
         "--out-csv", str((out_dir / "analysis" / "f19_robustness_table.csv").resolve())],
        cwd=str(HERE),
    )


def run_repeatability_suite(args, manifest_path, out_dir):
    """F21: opt-in only, since this multiplies real API cost by --repeatability-runs."""
    print(f"\n[run.py] F21: running the real pipeline {args.repeatability_runs}x for repeatability "
          f"(this makes real, billed Gemini calls) ...")
    run_prefix = out_dir / "uavi_sample_level"
    subprocess.run(
        [sys.executable, str(HERE / "run_repeatability.py"),
         "--manifest", str(Path(manifest_path).resolve()),
         "--n-runs", str(args.repeatability_runs),
         "--out-prefix", str(run_prefix.resolve())]
        + (["--max-samples", str(args.max_samples)] if args.max_samples else []),
        cwd=str(HERE), check=True,
    )

    run_files = [str((out_dir / f"uavi_sample_level_run{i}.json").resolve()) for i in range(1, args.repeatability_runs + 1)]
    print("\n[run.py] F21: measuring run-to-run agreement/variability ...")
    subprocess.run(
        [sys.executable, str(HERE / "analysis" / "f21_repeatability_analysis.py"), "--runs"] + run_files
        + ["--out-per-sample-csv", str((out_dir / "analysis" / "f21_per_sample_agreement.csv").resolve()),
           "--out-aggregate-csv", str((out_dir / "analysis" / "f21_aggregate_variability.csv").resolve())],
        cwd=str(HERE),
    )
...
def main():
    ...
    parser.add_argument("--run-corruption-suite", action="store_true",
                         help="F19: ALSO generate corrupted images and run a real second pipeline pass against them. "
                              "Costs real API money on top of the main run -- opt-in only.")
    parser.add_argument("--corruption-types", nargs="*", default=None,
                         choices=["gaussian_blur", "gaussian_noise", "brightness_shift", "jpeg_compress"],
                         help="Subset of corruption types for --run-corruption-suite (default: all 4)")
    parser.add_argument("--corruption-max-source-samples", type=int, default=None,
                         help="Cap source images before corrupting, for --run-corruption-suite (cost control)")

    parser.add_argument("--run-repeatability", action="store_true",
                         help="F21: ALSO run the real pipeline --repeatability-runs times to measure "
                              "run-to-run variability. Costs real API money on top of the main run -- opt-in only.")
    ...
    if args.run_corruption_suite:
        run_corruption_suite(args, manifest_path, out_dir)

    if args.run_repeatability:
        run_repeatability_suite(args, manifest_path, out_dir)
