...
import { InspectionChat } from '@/components/visioninspect/InspectionChat';
...
export default function VisionInspectPage() {
  ...
  const handleRetry = useCallback(() => {
    if (lastSubmission) {
      void runAnalysis(
        lastSubmission.imageBase64,
        lastSubmission.mimeType,
      );
    } else {
      setWorkflow({
        status: 'idle',
    ...
    <main className="mx-auto max-w-2xl px-4 py-8 sm:px-6 sm:py-12">
      <header className="mb-8">
        ...
        <p className="mt-2 max-w-prose font-body text-sm text-graphite-soft">
          Upload an inspection image to get an AI-proposed defect hypothesis
          with stated evidence and confidence. A reviewer must confirm or
          correct every finding before it becomes final.
        </p>
      </header>
      ...
      <section aria-label="New inspection" className="space-y-4">
        <InputForm
          ...
          disabled={workflow.status === 'analyzing'}
        />
        ...
        {workflow.status === 'result' && (
          <>
            ...
            {workflow.report && (
              <div className="mt-6">
                <InspectionChat report={workflow.report} />
              </div>
            )}
          </>
        )}
